...
<body>
    <div class="m-3">
        ...
        <div class="card p-3">
            <form action="{{ route('menus-deck.update', $menusDeck->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="status" name="status" required>
                        <option value="On-going" {{ $menusDeck->status == 'On-going' ? 'selected' : '' }}>On-going</option>
                        <option value="Planned" {{ $menusDeck->status == 'Planned' ? 'selected' : '' }}>Planned</option>
                        <option value="Done" {{ $menusDeck->status == 'Done' ? 'selected' : '' }}>Done</option>
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" id="tanggal_pelaksanaan" name="tanggal_pelaksanaan"
                        value="{{ $menusDeck->tanggal_pelaksanaan }}" required>
                </div>
